...
*/
SELECT status, COUNT(status) FROM Issue
GROUP BY status
...
*/
SELECT COUNT(Level.Manager) AS mlcc FROM Issue JOIN Staff ON Issue.Assigned_to = Staff.Staff_code JOIN Level ON Staff.Level_code = Level.Level_code
WHERE Level.Manager IS NOT NULL

--#4
/*
Show the manager for each shift. 
Your output should include the shift date and type; 
